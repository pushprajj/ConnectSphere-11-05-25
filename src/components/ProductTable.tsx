          ...
          {products.map((product, idx) => (
            <tr key={product.id || idx} className="bg-white hover:bg-gray-50 transition border-b last:border-0">
              ...
              <td className="px-4 py-2">
                <span className={`inline-block px-2 py-1 text-xs rounded font-semibold ${product.availability === 'Available' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>{product.availability}</span>
              </td>
              <td className="px-4 py-2 text-center">
                <div className="flex items-center gap-2 justify-center">
                  ...
                  >
                    ...
                    {product.id && openDropdownId === product.id && onDeleteProduct && (
